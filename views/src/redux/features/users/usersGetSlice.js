    ...
    return async (dispatch) => {
        try {
            if (typeof (img) !== "string") {
                if (img === undefined) return await instance.put('http://localhost:3001/user', { newValue })
                let res = await axios.post("https://api.cloudinary.com/v1_1/leo-echenique/image/upload", img);
